...
async function stokBoa() {
  await (async function () {
    ...
    const loadNotes = () => {
      try {
        const dataBuffer = fs.readFileSync("D:/web_scarper/data.json")// need revision
    ...
    const saveNotes = (num) => {
      ...
      fs.writeFileSync('D:/web_scarper/data.json', dataJSON) // need revision
    ...
    let json = JSON.parse(fs.readFileSync(path.join(__dirname, 'boa_data.json')));
